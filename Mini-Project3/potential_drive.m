%using repellent potential with perfect localization with no noise
clear all; close all
...
Kp1 = 0.3*eye(2*n); %gain for repel robots to certain distance
Kp2 = 0.4*eye(2*n); %gain for keep robots in the formation

%potential function
qrange=.05;
psifun = @(x,dx) (x<dx).*(1./x-1./dx);

Kp_lead = 0.3; %lead agent feed forward gain
...
function write_video(M,mode)
    %write movie to file:
    filename = 'potential_drive_v.avi';
    if mode == 2
        filename = 'potential_drive_circle.avi';
    end
