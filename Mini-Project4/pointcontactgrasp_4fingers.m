...
m = 4;
[X,FVAL,EXITFLAG,OUTPUT] = quadprog(G'*G,zeros(m,1),-eye(m),-ones(m,1));
[X1,FVAL1,EXITFLAG,OUTPUT] = quadprog(eye(m),zeros(m,1),-eye(m),-ones(m,1),G,zeros(3,1));
etaC = randn(3,1);
disp('without squeeze pinv(G)*etaC:')
eta_m = pinv(G)*etaC;
disp(eta_m);
if FVAL>.1;disp('^^^ NO FORCE CLOSURE GRASP =>Frictionless Contact ^^^');return;end
if isempty(X1);disp('^^^ NO FORCE CLOSURE GRASP => Frictionless Contact^^^');return;end
...
eta_a=pinv(G)*etaC+max(alpha_a)*X;
eta_b=pinv(G)*etaC+max(alpha_b)*X1;   


disp('Grasp map of a 4 frictionless point contact grasp:')
disp(G);
disp('null space of G')
disp(null(G)*a)
disp(eta_a)
disp(eta_b)
...
if FVAL>.1;disp('^^^ NO FORCE CLOSURE GRASP =>Frictional Contact ^^^');return;end
if isempty(X1);disp('^^^ NO FORCE CLOSURE GRASP => Frictional Contact^^^');return;end
...
f_contact=[rot2(q)*f(:,1)*eta(1)+rot2(-q)*f(:,1)*eta(2),...
    ...
    rot2(q)*f(:,3)*eta(5)+rot2(-q)*f(:,3)*eta(6),...
    rot2(q)*f(:,4)*eta(7)+rot2(-q)*f(:,4)*eta(8)];   
eta1=pinv(G1)*etaC+alpha1*X1;
f1_contact=[rot2(q)*f(:,1)*eta1(1)+rot2(-q)*f(:,1)*eta1(2),...
    rot2(q)*f(:,2)*eta1(3)+rot2(-q)*f(:,2)*eta1(4),...
    rot2(q)*f(:,3)*eta1(5)+rot2(-q)*f(:,3)*eta1(6),...
    rot2(q)*f(:,4)*eta1(7)+rot2(-q)*f(:,4)*eta1(8)];   
disp([eta,eta1]);
...
quiver(p03(1),p03(2),f_contact(1,3),f_contact(2,3),'linewidth',2);
quiver(p04(1),p04(2),f_contact(1,4),f_contact(2,4),'linewidth',2);
...
quiver(p03(1),p03(2),f1_contact(1,3),f1_contact(2,3),'--','linewidth',2);
quiver(p04(1),p04(2),f1_contact(1,4),f1_contact(2,4),'--','linewidth',2);
