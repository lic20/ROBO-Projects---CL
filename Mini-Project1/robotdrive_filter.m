...
qhat1(:,1)=pose_est_bf(y(:,1),pL,pB,N_scan,wcov,vcov);

Pk = 0.1*eye(3,3);
Sigmak = 0.1*eye(3,3);
[qhat2(:,1),Pk,Sigmak] = pose_est_filter(q(:,1),[0;0],y(:,1),ts,pL,pB,...
    N_scan,wcov,vcov,Pk,Sigmak,robot,colobj);
...
while keyvalue~='q'
    ...
    qhat1(:,k+1)=pose_est_bf(y(:,k+1),pL,pB,N_scan,wcov,vcov);
    [qhat2(:,k+1),Pk,Sigmak]=pose_est_filter(q(:,k),u(:,k),y(:,k),ts,pL,pB,N_scan,wcov,...
    vcov,Pk,Sigmak,robot,colobj);
    % check collision 
...
t=[1:k];
figure(2);plot(t,q,t,qhat1,':',t,qhat2,'^','linewidth',2);
legend('x','y','\theta','x_{est}','y_{est}','\theta_{est}','x_{EKF}',...
    'y_{EKF}','\theta_{EKF}');